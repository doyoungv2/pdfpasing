...
if not st.session_state.quiz_started:
    ...
    # 입력창
    user_answer = st.text_input("영문 풀네임과 뜻을 모두 입력하세요", key=f"q_{curr_idx}", disabled=st.session_state.submitted).strip()
...
st.markdown("<style>footer {visibility: hidden;} .stAppDeployButton {display:none;}</style>", unsafe_allow_html=True)
